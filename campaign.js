(() => {
  ...
  let qiTimer=null, qiPos=0, qiDir=1, qiLastFrame=0;
  ...
  function startTraining(){show('#trainingScreen');scene('sect');$('#chapterName').textContent='阳华云海 · 清平晨课';cs.training=0;$('#trainScore').textContent='○ ○ ○';$('#trainingHint').textContent='灵息靠近金色区域时点击“运气”';qiPos=0;qiDir=1;qiLastFrame=0;if(qiTimer)cancelAnimationFrame(qiTimer);const moveQi=now=>{if(!$('#trainingScreen').classList.contains('active')){qiTimer=null;return}if(qiLastFrame){qiPos+=qiDir*Math.min(3.2,(now-qiLastFrame)*.09);if(qiPos>=97){qiPos=97;qiDir=-1}else if(qiPos<=0){qiPos=0;qiDir=1}}qiLastFrame=now;$('#qiMarker').style.left=qiPos+'%';qiTimer=requestAnimationFrame(moveQi)};qiTimer=requestAnimationFrame(moveQi)}
  function train(){const good=qiPos>=36&&qiPos<=66;cs.training++;cs.cultivation+=good?2:1;$('#trainingHint').textContent=good?'周天圆满，额外获得一重修行。':'灵息稍有偏移，但江顾替你护住了经脉。';$('#trainScore').textContent=Array.from({length:3},(_,i)=>i<cs.training?'●':'○').join(' ');if(cs.training>=3){if(qiTimer)cancelAnimationFrame(qiTimer);qiTimer=null;cs.node++;setTimeout(()=>{persist();renderEpisode()},700)}}
